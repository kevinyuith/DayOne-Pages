...
function PageRow({
  ...
  const editHref = `/domains/${domainId}/pages/${page.id}`;
...
/** Replaces the page's content with a fresh copy of another template (the same one works too: back to the original). */
...
/**
 * Copy a template to the domain: the original, or a visual variation
 * generated on the spot (and, if asked, with the copy rewritten from a new
 * angle). The variation goes through a preview before reaching the domain.
 */
