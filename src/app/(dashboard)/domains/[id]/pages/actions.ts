...
/**
 * The editor opened on a domain page (domains.site). Same rules and same
 * response shape as the template actions (templates/actions.ts), so the
 * PageEditor doesn't need to know where the page comes from. Here a slug's id
 * is the path itself.
 *
 * Unlike templates, the domain page is live: every write clears the server
 * cache for the domain (if the purge fails, it takes effect when the cache
 * expires, within 30 s).
 *
 * The first two parts of each signature (domain, page) come from the
 * `.bind()` in the editor page.
 */
...
/** Saves the page's name/type/status and (if changed) the current slug's HTML. Zero rows from the database = `conflict`. */
