...
same('serve: ETag with the video', "\"vsl01-v$vB-b2\"", $headers['ETag']);
...
same('serve: 304 with the same video', 304, serve_slug($vslRoute, make_request(['HTTP_COOKIE' => "dop_vsl=$vB", 'HTTP_IF_NONE_MATCH' => "\"vsl01-v$vB-b2\""]))[0]);
same('serve: no 304 shortcut on the bare hash (the video may change)', 200, serve_slug($vslRoute, make_request(['HTTP_COOKIE' => "dop_vsl=$vB", 'HTTP_IF_NONE_MATCH' => '"vsl01-b2"']))[0]);
...
same('no split: ETag is just the hash', '"vsl01-b2"', $headers['ETag']);
