...
- **Tela Funil** (`/funil`): a lista dos funis do **dayone-main** (`public.funnels`, códigos F1, F2…), lidos só por `pages.main_funnels()` — o painel nunca escreve em `public`. Cada funil abre (recolhido por padrão) e mostra as páginas dele, uma linha por página: templates com `kind = 'FUNNEL'` ligados por `pages.pages.funnel_id` (FK para `public.funnels`, ON DELETE SET NULL; página sem funil aparece numa linha final). Os funis não entram na tela Templates. "New page" é o mesmo fluxo do "Create template" (`CreatePageForm` com `funnel`: copiar de template/link/HTML, pergunta dos marcadores, do zero = Pre Lander + Lander), e a página nasce com kind FUNNEL ligada ao funil. `folders.scope = 'FUNNEL'` existe no banco mas a tela Funil não usa pastas.
- **Teste A/B entre as páginas de um funil**: cada página tem o seu % (`pages.traffic_weight`, 0–100; 0 = pausada), editado na linha dela e lido AO VIVO pelas cópias (não é copiado). **Os % de um funil somam sempre 100** (`src/lib/pages/traffic.ts`, mesma conta na tela e na action): mudar um redistribui o resto entre as outras na proporção que tinham; página nova entra com 100/n e as outras encolhem; página apagada devolve a parte dela. "Copy funnel to domain" copia todas as páginas; no domínio, quando a rota resolve para a cópia de uma página de funil e há outras cópias do mesmo funil no mesmo path, `pages.resolve` devolve `split` e o servidor (`split_pick` em `funnel.php`) sorteia uma por visitante pelos pesos, fixa no cookie `dop_pg`; o hit registra a página servida. Números por página (`pages.page_stats`): carregamentos reais das cópias (`hit_loads`) e quantos clicaram para fora (`hit_loads.clicked_at`, o aviso de carregamento manda `c=1`). Mudar peso purga o cache dos domínios que têm o funil.
