...
- Com um **ângulo de copy** informado, os textos são reescritos por IA (`src/lib/pages/copy-angle.ts`): o Kimi (`kimi-k3`, API da Moonshot compatível com a da OpenAI, modo JSON) quando há `MOONSHOT_API_KEY`; senão o Claude (`@anthropic-ai/sdk`, `claude-opus-5`, streaming, saída estruturada, `fallbacks: "default"`) quando há `ANTHROPIC_API_KEY`. Só o texto entre as tags vai para o modelo e volta no mesmo lugar; trecho que perde um `{{marcador}}` fica como estava. O prompt proíbe inventar fatos, promessas, depoimentos e alegações. Sem nenhuma das chaves, só a variação visual funciona. Chaves só no `.env.local` / ambiente do servidor, nunca no repo (é público).
