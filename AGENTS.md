...
- **Tela Funil** (`/funil`): a lista dos funis do **dayone-main** (`public.funnels`, códigos F1, F2…), lidos só por `pages.main_funnels()` — o painel nunca escreve em `public`. Cada funil abre (recolhido por padrão) e mostra as páginas dele: templates com `kind = 'FUNNEL'` ligados por `pages.pages.funnel_id` (FK para `public.funnels`, ON DELETE SET NULL; página sem funil aparece numa linha final). Os funis não entram na tela Templates. O domínio recebe uma cópia da página do funil como de qualquer template (`domain_page_copy`). Clicar numa amostra abre o editor nela (`?sample=<id>`). `folders.scope = 'FUNNEL'` existe no banco mas a tela Funil não usa pastas.
